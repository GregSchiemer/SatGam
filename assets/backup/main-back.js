...
import { initHenge } from './henge.js';
...
//  isEndScreen = true;
//  renderEnd();    // uses the new signature
//}
...
//  installEndScreenTapHandler({ canvas, ctx });
  ...
  // 3) Show status object
    ...
    modeChosen:    false,                 // leader has tapped PREVIEW/CONCERT at least once
    modeConfirmed: (role !== 'leader'),   // consort skips mode-select
    role,                                  // 'leader' or 'consort'
...
//  };
  ...
  // Build + store slots
  const slots = await initHenge(ctx, n);

  // Build/resize atlas to match these slots
...
//  installLeaderModeSelectHandler(ctx, canvas, status);   // leader: choose preview vs concert
//  installLeaderModeConfirmHandler(ctx, canvas, status);  // leader: tap bottom text to accept
//  installLeaderStopHandler(ctx, canvas, status);         // leader: double-tap clock to stop
//  installClockStartHandler(ctx, canvas, status);         // everyone: tap clock to start
...
setRender(() => {
  ...
  if (currentRole === 'leader' && !status.modeConfirmed) {
    const mode =
